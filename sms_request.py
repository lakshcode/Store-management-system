...
#        print(s_id)
        ...
        if m==True:
            ...
            def reject():
                ...
                text="Dear member,\nYou are not recognized or authenticated by the store.\nFor more details visit store. \nThankyou"
            ...
            def cancel():
                ...
                cursor.execute("DELETE from deal WHERE d_id = ?",(did))
                conn.commit()
                remove_item()
                print("remove succesfully")
                supplier_label.set("")
            ...
            def adddeal():
                conn = pyodbc.connect('Driver={SQL Server};'
                     ...
                     'Trusted_Connection=yes;')
                cursor = conn.cursor()
                cursor.execute("UPDATE deal SET deal = ? WHERE d_id = ?",("True",did))
                conn.commit()
            ...
            def allow():
                ...
                subject="Your Are Verified By Store"
                text="Your Store Management id is "+email_l+" \npassword is "+p+" \n Thankyou"
                gmail(user, password, to_addrs, subject, text)
                cursor = conn.cursor()
                cursor.execute("DELETE from Requests WHERE id = ?",(idd))
                conn.commit()
