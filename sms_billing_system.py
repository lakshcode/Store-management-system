...
if mains==True:
    ...
    def take_id(idss,email,mains):
        ...
        if m==True:            
            ...
            def additem():
                ...
                if length:
                    for i in range(1,length+1):
                        ...
                        print(price)
                        order_S=1+order_S
                        order_tabel.insert("",'end',iid=order_S,values=(order_S,itemName.get(),itemVolume.get(),itemRate.get(),itemQuantity.get(),price,d_id))
                        totalamt()
            ...
            def bill():
                if(len(customerContact.get())==10 and customerName.get()!=""):                
                    length=len(order_tabel.get_children())
                    print(length)
                    flag=0
                    global order_S,item_id
                            ...
                            if answer:
                                for i in range(1,length+1):
                                    ...
                                    if rid[4]!=0:
                                        ...
                                        cursor.execute("Insert Into bill values(?,?,?,?,?,?,?)",(str(random.randint(100000,999999)),rid[6],rid[1],rid[5],rid[4],customerName.get(),customerContact.get()))
                                        conn.commit()
                    ...
                    mb.showerror("error","Please provide customer Name and Contact Number",parent=root)
            ...
            order_tabel = ttk.Treeview(order_tabel_frame,
                        columns =("S.No","name","Volume","rate","quantity","price","d_id"),xscrollcommand=scrollbar_order_x.set,
                        yscrollcommand=scrollbar_order_y.set)
