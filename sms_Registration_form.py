...
def register():
    ...
    def save():
        if(msg.flag==0):
            ...
            cur=conn.cursor()
            cur.execute("select Email from members where authenticated='True' and designation='Admin'")
            A_email=cur.fetchall()
            cur.close()
            if(A_email):
                if(designation=="Supplier"):
                    ...
                    mb.showinfo("Information", "Informing admin about you")
                    gmail(user, password, A_email, "New Supplier", text)
    ...
    def saveinfo():
        ...
        if(msg.flag==0):
            ...
            if(otp_var.get()==""):
                ...
                save()
                root.destroy()
                from sms_signup_page import mains
                mains()
            else:
    ...
    def defaultquery():
        ...
        if(row):
            ...
            cursor = conn.cursor()
            cursor.execute("INSERT INTO members VALUES('000001','Store','Management',?,'NULL','7703920989','storemanagementsystem.sla@gmail.com','NULL','00000','Admin','True',?)",(now,now))
            conn.commit()
